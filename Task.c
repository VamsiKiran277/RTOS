...
uint32_t* TCB_Initialization(void(*task)(void),uint32_t*stack_arr) {
    ...
    topOfStack[-9]  = 0x11111111; // R11 fake tracking value
    topOfStack[-10] = 0x10101010; // R10 fake tracking value
    topOfStack[-11] = 0x09090909; // R9 fake tracking value
    topOfStack[-12] = 0x08080808; // R8 fake tracking value
    topOfStack[-13] = 0x07070707; // R7 fake tracking value
    topOfStack[-14] = 0x06060606; // R6 fake tracking value
    topOfStack[-15] = 0x05050505; // R5 fake tracking value
    topOfStack[-16] = 0x04040404; // R4 fake tracking value
